...
-- Create new policy that only allows updating name field for own profile
...
-- Create admin-only role update policy (separate from self-updates)
CREATE POLICY "user_profiles_update_role_admin_only" 
ON public.user_profiles 
FOR UPDATE 
USING (
  auth.uid() != user_id  -- Admin updating someone else's profile
  AND EXISTS (
